...
describe("sampleResourceSignals", () => {
  ...
  it("uses host PSI only when no cgroup memory controller is available", async () => {
    const fs = mapFs([
      ["/proc/self/cgroup", "malformed\n"],
      ["/proc/self/mountinfo", "malformed\n"],
      [
        "/proc/pressure/memory",
        "some avg10=2.50 avg60=2.00 avg300=1.00 total=9\nfull avg10=0.00 avg60=0.00 avg300=0.00 total=0\n",
      ],
    ]);
    const signals = await sampleResourceSignals({
    ...
    assert.equal(signals.psi?.someAvg10, 2.5);
  });
